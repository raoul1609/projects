        ...
        if formulaire.is_valid():
            ...
            try:
                ...
                if type(checkExistantCourse) == Programme:
                    messages.error(
                        request,
                        "Il y a deja un cours programme a la date du "
                        + str(dateVerified),
                    )
                ...
                formToSave = Programme.objects.create(
                    jour=dateVerified,
                    cours=intituleCours,
                    enseignant=nameTeaherVerified,
                    teacherTel=lookForTeacher.telephone,
                )
                messages.success(
                    request, "Ok, le programme a ete enregistre avec succes"
                )
                return render(request, "new_saveNewProgram.html", {})
        else:
            formulaire = ProgammeForm()
            context = {
                "titre": "Definir Programme",
...
def addTeacher(request):
    ...
    if request.method == "POST":
        ...
        if teacherForm.is_valid():
            ...
            try:
                checkExistanceOfTeacher = get_object_or_404(
                    MiawTeachers, pk=verifiedTeacherName
                )
                ...
                newTeacher = MiawTeachers.objects.create(
                    nom=verifiedTeacherName, telephone=verifiedPhone
                )
...
def lookForOneProgram(request, name):
    ...
    if name:
        try:
            lookupProgramInDb = Programme.objects.raw(
                "SELECT * FROM emploi_du_temps_programme where cours = %s", [name]
            )
